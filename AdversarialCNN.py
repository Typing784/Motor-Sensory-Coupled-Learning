...
from tensorflow.keras.layers import AveragePooling2D, Conv2D, DepthwiseConv2D, SeparableConv2D  # 修复了这个导入问题
...
tf.keras.backend.set_image_data_format('channels_last')  # TensorFlow 2.x 中的后端设置
class AdversarialCNN:
    def __init__(self, chans, samples, n_output, n_nuisance, architecture='DeepConvNet', adversarial=False, lam=0.01):
        ...
        if adversarial:
            self.acnn.compile(loss=[lambda x, y: tf.keras.losses.categorical_crossentropy(x, y, from_logits=True),
                                    lambda x, y: tf.keras.losses.categorical_crossentropy(x, y, from_logits=True)],
                              loss_weights=[1., -1. * self.lam], optimizer=Adam(learning_rate=1e-3, decay=1e-4),
                              metrics=['accuracy'])
        else:   # trains a regular (non-adversarial) CNN, but will monitor leakage via the adversary alongside
            self.acnn.compile(loss=[lambda x, y: tf.keras.losses.categorical_crossentropy(x, y, from_logits=True),
                                    lambda x, y: tf.keras.losses.categorical_crossentropy(x, y, from_logits=True)],
                              loss_weights=[1., 0.], optimizer=Adam(learning_rate=1e-3, decay=1e-4),
                              metrics=['accuracy'])

        self.adv.trainable = True
        self.adv.compile(loss=lambda x, y: tf.keras.losses.categorical_crossentropy(x, y, from_logits=True),
    ...
    def encoder_model(self, architecture):
        ...
        if architecture == 'EEGNet':
            ...
            model.add(Activation(lambda x: tf.log(tf.clip_by_value(x, clip_value_min=1e-7, clip_value_max=10000))))
            model.add(Dropout(0.5))
            model.add(Flatten())
